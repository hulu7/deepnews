		...
		if (catalog === "all") {
			articleModel.paginate({subscribe:{$in:['dn201900001']}}, {page: page, limit: limit, sort:{published:-1}}, function(err, result) {
				result.docs.forEach(doc => {
					result.docs.forEach(doc => {
						if (doc._doc['clickUpdate'] != undefined)
							doc._doc.clickVolume = doc._doc.clickUpdate.length;
						delete doc._doc.clickUpdate;
...
exports.viewedArticle = function(req, res) {
	try {
		formParse.formidableFormParse(req,function(err,result){
			...
			articleModel.find({language:'ch', _id:{$in:[id]},subscribe:{$in:[username]}},function(err,result){
				if (result.length > 0) {
					...
					var updateTime = date.getFullYear() + "-" +
						...
						date.getMinutes() + ":" +
						date.getSeconds();
					result[0]._doc.clickUpdate.push(updateTime);
					articleModel.update({_id: result[0]._doc._id},result[0],function(err,result){
					});
				}
			});
		});
	} catch (e) {
		console.log('Exception to click update error:' + e);
	}
}
...
exports.searchArticles = function(req,res){
	try {
		...
		console.log("Exception to search articles: " + e);
	}
}
